...
| [`separate-services`](../../tree/separate-services) | The Complete example from [`REFERENCE.md`](./REFERENCE.md) re-expressed as **one service per extension** instead of a single `host: microsoft.foundry` entry: an `azure.ai.projects` service (project + model deployments), plus `azure.ai.connections`, `azure.ai.toolboxes`, and `azure.ai.skills` services, and **one `azure.ai.agents` service per agent** (per-agent services, as today). Services are ordered with `uses:`; resources reference each other by name across service boundaries. Demonstrates Option B (per-resource extensions own their slice) from the design brief below. |
